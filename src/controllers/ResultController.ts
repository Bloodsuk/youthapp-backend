...
/**
 * Get extracted PDF text for an order attachment.
 */
interface GetPdfTextReqBody {
  userData: ISessionUser
}

async function getPdfText(req: IReq<GetPdfTextReqBody>, res: IRes) {
  ...
  try {
    const text = await ResultService.extractPdfTextByOrderId(id, userData.id);
    return res.status(HttpStatusCodes.OK).json({ text }).end();
  } catch (error) {
    if (error instanceof RouteError)
      return res
        .status(error.status)
