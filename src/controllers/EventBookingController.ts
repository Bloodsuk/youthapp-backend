...
import EventBookingService from "@src/services/EventBookingService";
...
/**
 * GET /event_bookings/pleb/:pleb_id
 */
async function getByPlebId(req: IReq, res: IRes) {
  ...
  if (
    ...
    return res
      ...
      .json({
        success: false,
        error: "You can only view your own event bookings",
      })
      .end();
  }

  try {
    const data = await EventBookingService.getByPlebId(plebId);
    return res
...
/**
 * POST /event_bookings/update_status/:id
...
async function updateStatus(
  ...
  if (!Number.isFinite(id) || id <= 0) {
    return res
      .status(HttpStatusCodes.BAD_REQUEST)
      .json({ success: false, error: "Invalid event booking id" })
      .end();
  }
