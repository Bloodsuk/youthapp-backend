...
async function practitionerHasMessagesByCustomer(
  ...
  const customerIds = rows?.map((el: any) => el.id);
  sql = `SELECT sent_from as sent_by_customer_id, 'unread' as message_type FROM messages WHERE sent_to = ? AND sent_from IN (?) AND is_read = 0 Group by sent_from`;
